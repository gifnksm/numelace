...
impl Game {
    ...
    pub fn from_problem_filled_notes(
        ...
        for (row_index, row) in (0..9).zip(notes) {
            for (col_index, bits) in (0..9).zip(row) {
                let pos = Position::new(row_index, col_index);
                let digits =
    ...
    fn test_apply_technique_step_eliminates_notes_only() {
        ...
        impl TechniqueStep for TestStep {
            ...
            fn application(&self) -> Vec<TechniqueApplication> {
                ...
                positions.insert(Position::new(0, 1));
        ...
        game.toggle_note(Position::new(0, 0), Digit::D5, RuleCheckPolicy::Permissive)
            .unwrap();
        game.toggle_note(Position::new(0, 1), Digit::D5, RuleCheckPolicy::Permissive)
            .unwrap();
        assert!(matches!(
        ...
        assert_eq!(game.cell(Position::new(0, 1)), &CellState::Empty);
    }
